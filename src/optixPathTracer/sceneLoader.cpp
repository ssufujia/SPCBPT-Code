...
std::vector<LightParameter> breakLight(LightParameter& a,int divLevel)
{
	...
	optix::float3 sig_v1 = a.u / divLevel;
	optix::float3 sig_v2 = a.v / divLevel;
...
Scene* LoadScene(const char* filename)
{
	...
	while (fgets(line, kMaxLineLength, file))
	{
        ...
        if (strstr(line, "cameraSetting"))
        {
            ...
            optix::float3 eye, lookat,up = optix::make_float3(0,1,0);
			std::string env_file = "CedarCity.hdr";
            float fov = 35.0f;
			int geo_normal = 0;
			float env_lum = 1;
            while (fgets(line, kMaxLineLength, file))
            {
				...
				sscanf(line, " env_file %s", &env_file); 
            }
			if (geo_normal == 1)
			{
		...
		if (strstr(line, "mesh"))
		{
            optix::Matrix4x4 xform = optix::Matrix4x4::identity();// (m_data);
			while (fgets(line, kMaxLineLength, file))
			{
				// end group
                if (strchr(line, '}'))
				...
				if (sscanf(line, " file %s", path) == 1)
				{
					...
					scene->uv_mesh_names.push_back(std::string(sutil::samplesDir()) + "/data/" + path);
				}
				if (sscanf(line, " uv_file %s", path) == 1)
				{ 
					scene->uv_mesh_names.back() = (std::string(sutil::samplesDir()) + "/data/" + path);
				}
                ...
                if (strstr(line, "transform" ))
                {
                    ...
                    xform = optix::Matrix4x4(m_data).transpose();
